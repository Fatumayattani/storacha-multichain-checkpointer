...
async function checkGateway(
  ...
  try {
    ...
    console.log(
      `Gateway ${gateway}: ${response.ok ? "✅" : "❌"} ${response.status} (${responseTime}ms)`
    );
    return result;
  } catch (error) {
    const responseTime = Date.now() - startTime;
...
export async function verifyCIDAvailability(
  ...
  if (
    cleanCID.length < 10 ||
    !/^(Qm[1-9A-HJ-NP-Za-km-z]{44}|bafy[a-z2-7]{52}|bafk[a-z2-7]{52})/.test(
      cleanCID
    )
  ) {
  ...
  const successfulChecks = gatewayChecks.filter(
 ...
 */
export function getFastestGateway(
  verificationResult: CIDVerificationResult
): GatewayCheck | null {
  const availableGateways = verificationResult.gateways.filter(
    (g) => g.available
  );
...
export function useCIDVerification() {
  ...
  const [verificationResult, setVerificationResult] =
    React.useState<CIDVerificationResult | null>(null);
