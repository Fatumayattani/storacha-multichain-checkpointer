  ...
  const initializeClient = useCallback(async () => {
    try {
      ...
      if (!currentSpace) {
        ...
        console.log('⚠️  Space setup required. This might require email login...')
        ...
        try {
          ...
          console.log('⚠️  Space creation requires account. Error:', spaceError)
...
- Or set up proper Storacha account first`
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to initialize Storacha client'
  ...
  const uploadFile = useCallback(async (file: File): Promise<UploadResult> => {
    ...
    try {
      ...
      if (!currentSpace) {
        throw new Error('No current space available. Try re-initializing the client.')
      }
      
      console.log('🔄 Uploading file:', file.name, 'to space:', currentSpace.did())
  ...
  const uploadMultipleFiles = useCallback(async (files: FileList | File[]): Promise<UploadResult[]> => {
    if (!client) {
      throw new Error('Storacha client not initialized')
    }
